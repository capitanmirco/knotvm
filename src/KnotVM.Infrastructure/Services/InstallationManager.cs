...
public class InstallationManager : IInstallationManager
{
    ...
    public void RenameInstallation(string fromAlias, string toAlias)
    {
        ...
        try
        {
            ...
            _installationsRepo.Add(installation with { Alias = toAlias, Path = newPath });
        ...
        {
            throw new KnotVMException(KnotErrorCode.InstallationFailed, 
                $"Errore rinominando installazione da '{fromAlias}' a '{toAlias}': {ex.Message}", ex);
        }
    }
    ...
    public void RemoveInstallation(string alias, bool force = false)
    {
        ...
        {
            throw new KnotVMException(KnotErrorCode.InvalidAlias, 
    ...
    public bool IsAliasValid(string alias) =>
        ...
        alias.Length is >= 1 and <= 50 &&
        AliasRegex.IsMatch(alias) &&
        !ReservedAliases.Contains(alias);

    public bool AliasExists(string alias) =>
        !string.IsNullOrWhiteSpace(alias) && _installationsRepo.GetByAlias(alias) != null;
